...
def main():
    console.print(Panel(
        ...
        f"[dim]日志文件: {config_info['log_file']}[/dim]",
        title="欢迎使用",
        border_style="blue"
    ))
    ...
    if not Confirm.ask("[bold green]是否开始处理?[/bold green]",default=True):
    ...
    if args.mode == 'image':
        for directory in items_to_process:
            try:
                DirectoryProcessor(console).process_directory(directory)
        ...
        for zip_path, input_base_path in items_to_process:
            try:
                ZipProcessor().process_zip(zip_path, input_base_path)
                stats.increment_processed()
            except Exception as e:
