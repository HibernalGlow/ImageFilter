        ...
        with tab3:
            ...
            for size, size_duplicates in duplicates.items():
                with st.expander(f"📋 哈希尺寸 {size} - 发现 {len(size_duplicates)} 对疑似重复", expanded=len(size_duplicates) > 0):
                    if size_duplicates:
                        for i, pair in enumerate(size_duplicates):
                            img1_path = Path(pair["image1"])
                            img2_path = Path(pair["image2"])
                            
                            st.markdown(f"**疑似重复 {i+1}** - 汉明距离: {pair['distance']}")
                            
                            col1, col2, col3 = st.columns([2, 2, 1])
                            with col1:
                                if img1_path.exists():
                                    st.image(str(img1_path), caption=img1_path.name, width=150)
                                st.caption(f"📁 {img1_path.parent}")
                            
                            with col2:
                                if img2_path.exists():
                                    st.image(str(img2_path), caption=img2_path.name, width=150)
                                st.caption(f"📁 {img2_path.parent}")
                            
                            with col3:
                                st.metric("相似度", f"{100-pair['distance']*2:.1f}%")
